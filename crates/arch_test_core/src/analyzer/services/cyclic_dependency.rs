...
pub fn contains_cyclic_dependency_on_level(module_tree: &ModuleTree, level: usize) -> bool {
    ...
    module_tree.tree().iter().enumerate().filter(|(_, node)| *node.level() == level).for_each(|(index, node)| {
        let included_nodes: Vec<usize> = node.included_nodes(module_tree.tree());
