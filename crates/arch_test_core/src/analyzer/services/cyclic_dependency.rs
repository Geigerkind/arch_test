...
fn find_traverse(visited_nodes: &mut Vec<UseRelation>, current_index: usize, module_tree: &ModuleTree) -> bool {
    if visited_nodes.iter().any(|use_relation| use_relation.using_object().node_index() == current_index) {
        return true;
    }
...
pub fn contains_cyclic_dependency_on_any_level(module_tree: &ModuleTree) -> Option<Vec<UseRelation>> {
...
pub fn contains_cyclic_dependency_on_level(module_tree: &ModuleTree, level: usize) -> Option<Vec<UseRelation>> {
...
fn find_traverse_on_level(visited_nodes: &mut Vec<UseRelation>, current_index: usize, node_mapping: &HashMap<usize, usize>, use_relations_per_level: &HashMap<usize, Vec<UseRelation>>) -> bool {
    if visited_nodes.iter().any(|use_relation| use_relation.using_object().node_index() == current_index) {
        return true;
    }
